# Olá! Eu sou o Davi Araújo 👋

🎓 Estudante de Ciência de Dados na Universidade Federal do Ceará (UFC)

💻 Interesse em Análise de Dados, Estatística e Tecnologia

🐍 Python | 📊 Pandas | 📈 Visualização de Dados | 🗄️ MySQL | 📐 R

---

## 👨‍💻 Sobre mim

Sou estudante de Ciência de Dados na Universidade Federal do Ceará (UFC), com interesse em análise de dados, programação, estatística e tecnologia.

Ao longo da minha formação, venho desenvolvendo conhecimentos em Python, R, estatística, bancos de dados e visualização de dados, além de experiências práticas com manutenção de sistemas e desenvolvimento de projetos de análise de dados.

Também tive experiências no Núcleo de Aprendizagem Cooperativa (NAC), no Programa de Educação Tutorial (PET) de Estatística e na Gauss Empresa Júnior de Estatística, onde desenvolvi habilidades de trabalho em equipe, comunicação, organização e resolução de problemas.

Atualmente, estou em busca de uma oportunidade de estágio onde possa aplicar meus conhecimentos, aprender com novos desafios e contribuir para projetos reais.

---

## 🚀 Projeto em destaque

### 🧠 Tecnologia e Saúde Mental

Projeto de análise exploratória de dados e visualização sobre a relação entre tecnologia, ambiente de trabalho e saúde mental.

Tecnologias utilizadas:

- Python
- Pandas
- Plotly
- Streamlit
- Jupyter Notebook

O projeto conta com um dashboard interativo que permite explorar os dados utilizando filtros e diferentes visualizações.

🔗 [Ver projeto no GitHub](https://github.com/Davyeeh/Tecnologias-e-sa-de-de-mental)

🌐 [Acessar Dashboard](https://saude-tecnologia-ufc.streamlit.app/)

---

## 🛠️ Tecnologias e ferramentas

🐍 Linguagens

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=for-the-badge&logo=r&logoColor=white)

📊 Dados e visualização

![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=for-the-badge&logo=matplotlib&logoColor=white)
![Seaborn](https://img.shields.io/badge/Seaborn-4C72B0?style=for-the-badge&logo=python&logoColor=white)
![Plotly](https://img.shields.io/badge/Plotly-3F4F75?style=for-the-badge&logo=plotly&logoColor=white)

🗄️ Banco de dados e desenvolvimento
 
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)

📈 Outras ferramentas

"Power BI" (https://img.shields.io/badge/Power%20BI-Básico-F2C811?style=for-the-badge&logo=powerbi&logoColor=black)
"Excel" (https://img.shields.io/badge/Excel-Intermediário-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white)

---

## 💼 Experiência

Núcleo de Aprendizagem Cooperativa (NAC) — UFC

Bolsista BIA — Suporte Técnico e Monitoria | 2024–2025

- Manutenção do site institucional.
- Atualizações de front-end utilizando HTML.
- Ajustes pontuais de back-end utilizando PHP.
- Suporte em consultas e conexão com banco de dados MySQL.
- Participação em atividades de aprendizagem cooperativa.

Programa de Educação Tutorial (PET) — Estatística | UFC

Membro Bolsista | 2022

- Atuação em ensino, pesquisa e extensão.
- Participação em eventos acadêmicos e grupos de estudo.
- Desenvolvimento de habilidades de comunicação e organização de projetos.

Gauss Empresa Júnior de Estatística — UFC

Membro Trainee | 2022–2023

- Vivência em ambiente empresarial júnior.
- Participação em atividades de consultoria estatística.
- Desenvolvimento de competências comerciais, negociação e gestão de tempo.

---

## 🎓 Formação

Universidade Federal do Ceará — UFC

🎓 Bacharelado em Ciência de Dados

📅 Previsão de conclusão: Dezembro de 2028

---

## 📜 Cursos e certificações

- Inglês — Centro Cearense de Idiomas (CCI)
- Microsoft Power BI — Básico — Fundação Bradesco
- Responsive Web Design — freeCodeCamp
- Marketing Pessoal — CEBRAC
- Técnico em Informática — EEEP José de Barcelos

---

## 📫 Contato

📧 E-mail: daviaraujocontato9@gmail.com

💼 LinkedIn: [Meu LinkedIn](https://www.linkedin.com/in/davi-da-silva-araújo-616454253?utm_source=share_via&utm_content=profile&utm_medium=member_android)

🐙 GitHub: [@Davyeeh](https://github.com/Davyeeh)

---

⭐ Obrigado por visitar meu perfil!